...
class EditableGrid {
  ...
  public function addColumn($name, $label, $type, $values = NULL,
			    $flags = EG_EDIT, $field = NULL, $bar = true) {
  ...
  public function getXML($rows=false, $customRowAttributes=false,
	...
	$columnNode->setAttribute('hidden', (int)(($info['flags'] & EG_HIDE) > 0));
  ...
  public function renderXML($rows=false, $customRowAttributes=false,
			    $encodeCustomAttributes=false, $includeMetadata=true) {
    header('Content-Type: text/xml');
    echo $this->getXML($rows, $customRowAttributes, $encodeCustomAttributes, $includeMetadata);
  }

  public function getJSON($rows=false,
			  $customRowAttributes=false,
			  $encodeCustomAttributes=false,
    ...
    $results['data'] = array();
    if ($rows) {
      $fetchMethod = method_exists($rows, 'fetch')
	? 'fetch'
	: (method_exists($rows, 'fetch_assoc')
	   ? 'fetch_assoc'
	   : (method_exists($rows, 'FetchRow')
	      ? 'FetchRow'
	      : NULL));

      if (!$fetchMethod) {
	foreach ($rows as $row) {
	  $results['data'][] = $this->getRowJSON($row, $customRowAttributes, $encodeCustomAttributes);
	}
      }
      else {
